...
"""
    2. Extracting Input/Output matrices for training

    extracts two numpy matrices representing input output pairs
    will have to modify this file to change the input representation to the neural network
...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        """
            return a list of POS Tags in the input sentence and an instance of class state

            sigma = state.stack
            beta = state.buffer
        """

        special_tokens = defaultdict()
        special_tokens['CD'] = 0
        special_tokens['NNP'] = 1
        special_tokens['UNK'] = 2
        special_tokens[None] = 3
        special_tokens['NULL'] = 4

        stack_words = [
            state.stack[-1] if len(state.stack) > 0 else 'NULL',
            state.stack[-2] if len(state.stack) > 1 else 'NULL',
            state.stack[-3] if len(state.stack) > 2 else 'NULL'
        ]

        buffer_words = [
            state.buffer[-1] if len(state.buffer) > 0 else 'NULL',
            state.buffer[-2] if len(state.buffer) > 1 else 'NULL',
            state.buffer[-3] if len(state.buffer) > 2 else 'NULL'
        ]
        
        context_words = stack_words + buffer_words
        input_list = np.zeros(6,dtype=np.int64)

        for idx in range(6):
            list_current = context_words[idx]
            top_pos = pos[list_current] if list_current != 'NULL' else 'NULL'
            if top_pos in special_tokens:
                input_list[idx] = special_tokens[top_pos]
            else:
                word = words[list_current].lower()
                if word in self.word_vocab:
                    input_list[idx] = self.word_vocab[word]
                else:
                    input_list[idx] = special_tokens['UNK']
    ...
    def get_output_representation(self, output_pair):  
        ...
        """
            ...
            output_pair:: transition
            self.output_labels[output_pair] :: index
            number of classes:: 91
        """

        index = self.output_labels[output_pair]
        output_matrix = np.zeros(91, dtype = np.float64)
        output_matrix[index] = 1.0
